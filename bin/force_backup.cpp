...
void force(const IonCloud &_cloud,_ode_vars &odev){
    ...
    for(unsigned j_=0; j_< _cloud.nrparticles; j_++){
        ...
        if(trap_config == 4){
            ...
            //cout << sqrt(x*x+y*y) << " " << z << " " << (*forcev).Potential_map.GetPotential(x,y,z) << endl;
                ...
                }	//cross product :: a x b = (a2b3 - a3b2) i + (a3b1-a1b3) j + (a1b2 - a2b1) k
                	qDmassa= _cloud.charge[j_]*el_charge/_cloud.mass[j_];
	...
	//excitation computation
		...
		double delaytime = odev.w_exc;//1./320000.*0.5;
		double resistance = 1294.;
		double amplification = 1000.; //60 dB: 25*2 (ZFL) + 10 (custom amp).
		double correction_factor = odev.U_exc;
		Asinwt = _cloud.GetImageCurrent(delaytime)*resistance*amplification*correction_factor;
	
		qDmassa= _cloud.charge[j_]*el_charge/_cloud.mass[j_];
		e_field=ARfieldmap.getField(x,y,z);
...
void InitARexcitation(char *Erz_filename){
   ARfieldmap.ReadField(Erz_filename);
   tmpstream.open("AR.txt");
   tmpinterval = 0.0;
   //ARfieldmap.PrintOnAxis();
