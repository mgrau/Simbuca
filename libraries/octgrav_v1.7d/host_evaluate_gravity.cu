...
extern "C"
{
  ...
  void initCUDA() {   
 fprintf(stderr, " -> Using the octtree code v1.7d \n");
	//:qcudaSetDevice(1);      
...
//     allocateCUDAarray((void**)&dev_interaction_list_len,  n_cells * sizeof(int3));
    ...
    double t1 = get_time();
//C    double dt_ilen = 0;
//C    fprintf(stderr, "   compute_interaction_list_len ... ");
    dev_compute_interaction_list_len<<<grid, threads>>>(dev_interaction_list_len);
    threadSync();
 //C   fprintf(stderr, "   done in %lf seconds \n", (dt_ilen = get_time() - t1));
...
//C    fprintf(stderr, " *****************************************************\n");
//C  fprintf(stderr, "       n_blocks= %d  n_in_block= %d\n", n_blocks, p*NBLOCKS);
//C    fprintf(stderr, "   #interacting nodes=  %d [max in block= %d]\n", n_interacting_nodes_total, n_interacting_nodes_max);
//C    fprintf(stderr, "   #interacting leaves= %d [max in block= %d]\n", n_interacting_leaves_total, n_interacting_leaves_max);
//C    fprintf(stderr, "   read+write_len= %lg GB  (%lg GB/s)\n", 
//C	    n_io*4.0/pow(1024.0,3.0),
//C	    n_io*4.0/pow(1024.0,3.0)/dt_ilen);
//C    fprintf(stderr, " *****************************************************\n");
...
//C      fprintf(stderr, "   block %d out of %d\n", cur_block, n_blocks);
...
//        fprintf(stderr, "   grid= [%d %d %d]\n", grid.x, grid.y, grid.z);
      ...
      double dt_ibuild0 = 0;
      t1 = get_time();
//C      fprintf(stderr, "   build_interaction_list ... ");
...
//C      fprintf(stderr, "   done in %lf seconds [%lf] \n", dt_ibuild0, dt_ibuild);
      ...
      double dt_node0 = 0;
//C      fprintf(stderr, "   evaluate_gravity_node  ... "); t1 = get_time();
 ...
 //C     fprintf(stderr, "   done in %lf seconds [%lf] \n", dt_node0, dt_node);
      ...
      double dt_leaf0 = 0;
//C      fprintf(stderr, "   evaluate_gravity_leaf  ... "); t1 = get_time();
...
//C    double flops_per_node = 57;
//C    double flops_per_leaf = 21;
    

//C    double flops_per_node1 = 57 + 16;
//C    double flops_per_leaf1 = 21 + 16;
#ifdef OCTUPOLE
//C    flops_per_node  += 108;
//C    flops_per_node  += 108;
//C    flops_per_node1 += 108;
//C    flops_per_node1 += 108;
#endif
...
//C    fprintf(stderr, "   read+write_bld= %lg GB  (%lg GB/s)\n", 
//C	    n_io*4.0/pow(1024.0,3.0),
//C	    n_io*4.0/pow(1024.0,3.0)/dt_ibuild);
   
//C    fprintf(stderr, "    interaction statistics: \n");
//C    fprintf(stderr, "      n_nodes=  %ld (%lg [%lg] GLFOPS)\n", n_node, n_node*flops_per_node/dt_node/1e9, n_node*flops_per_node1/dt_node/1e9);
//C    fprintf(stderr, "      n_leaves= %ld (%lg [%lg] GFLOPS)\n", n_leaf, n_leaf*flops_per_leaf/dt_leaf/1e9,  n_leaf*flops_per_leaf1/dt_leaf/1e9);
//C    fprintf(stderr, " *****************************************************\n");
