...
void octgrav::evaluate_gravity(vector<float4> &bodies_pos_in,
  ...
  double t1   = get_time();
//C  fprintf(stderr, "Load data ... ");
  float4 com = load_data(bodies_pos_in);
//C  fprintf(stderr, "done in %lf sec \n", get_time() - t1);
//C  fprintf(stderr, "system_com= [%g %g %g %g] \n\n",
//C	  com.x, com.y, com.z, com.w);
...
//C  fprintf(stderr, "\n\n old_tree= %d \n rebuild_tree= %d\n\n", old_tree, rebuild_tree);
  ...
  if (old_tree == false || rebuild_tree == true) {
    t1   = get_time();
//C    fprintf(stderr, "Building gravity tree ... \n");
    tree.destroy_tree();
...
//C    fprintf(stderr, "done in %lf sec \n", get_time() - t1);
  ...
  while(repeat) {
    ...
    t1 = get_time();
//C    fprintf(stderr, "Compute multipole moments ... ");
...
//C  fprintf(stderr, "root_com= [%g %g %g %g] \n",
//C	  root_com.x, root_com.y, root_com.z, root_com.w);
  
  t1 = get_time();
//C  fprintf(stderr, "Generating oct_tree lists ... \n");
  child_struct child;
...
//C  fprintf(stderr, "done in %lf sec \n", get_time() - t1);
  ...
  t1 = get_time();
//C  fprintf(stderr, "Reorder bodies ... ");
...
//C  fprintf(stderr, "  children_list.size= %d \n", (int)children_list.size());
//C  fprintf(stderr, "  node_list.size=     %d \n", (int)node_list.size());
//C  fprintf(stderr, "  leaf_list.size=     %d \n", (int)leaf_list.size());
//C  fprintf(stderr, "  cell_list.size=     %d \n", (int)cell_list.size());

  t1 = get_time();
//C  fprintf(stderr, "Allocating host memory ... ");
  allocate_host_memory();
//C  fprintf(stderr, "done in %lf sec \n", get_time() - t1);
  t1 = get_time();
//C  fprintf(stderr, "Allocating device memory ... ");
 ...
 //C fprintf(stderr, "done in %lf sec \n", get_time() - t1);
  ...
  t1 = get_time();
//C  fprintf(stderr, "Preparing & sending data to the device ... ");
  prepare_data_for_device();
  copy_data_to_device();
//C  fprintf(stderr, "done in %lf sec \n", get_time() - t1);

  double dt_cuda = CUDA_evaluate_gravity();
//C  fprintf(stderr, "dt_cuda= %lf\n", dt_cuda);
  
  t1 = get_time();
//C  fprintf(stderr, "Copying data form the device ... ");
...
//      bodies_grav[i] = hst.bodies_grav[i];
  ...
  t1 = get_time();
//C  fprintf(stderr, "Freeing memory ... ");
  free_cuda_memory();
  free_host_memory();
//C  fprintf(stderr, "done in %lf sec \n", get_time() - t1);
